...
export const ProjectsModal: React.FC<ProjectsModalProps> = ({
  ...
  const [tabFilter, setTabFilter] = useState<'all' | 'my'>('all');
  ...
  const isOwnerMatch = (jobOwnerRaw?: string) => {
    if (!currentLdap || !jobOwnerRaw) return false;
    const owner = jobOwnerRaw.trim().toLowerCase();
    const curr = currentLdap.trim().toLowerCase();
    if (owner === curr) return true;
    const ownerPrefix = owner.split('@')[0];
    const currPrefix = curr.split('@')[0];
    return ownerPrefix === currPrefix;
  };

  const myJobsCount = currentLdap
    ? jobs.filter((j) => isOwnerMatch(j.creatorLdap)).length
    : 0;

  const filteredJobs = jobs.filter((j) => {
    if (tabFilter === 'my' && currentLdap) {
      if (!isOwnerMatch(j.creatorLdap)) {
        return false;
      }
    }
    ...
    return (
      ...
      (j.creatorLdap || '').toLowerCase().includes(query) ||
    ...
    if (job.readyVideosCount > 0) {
      return (
        <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-500/30 flex items-center gap-1.5 shadow-sm">
          ...
          <span>Scenes Ready ({job.readyVideosCount}/10)</span>
        </span>
      );
    }
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md overflow-y-auto animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden my-auto animate-scaleUp">
        ...
        <div className="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <div className="flex items-center gap-2.5">
            {showBulkDelete && jobs.length > 0 && onBulkDeleteAllJobs && (
              <div className="flex items-center gap-1.5 p-1 rounded-xl bg-rose-500/10 border border-rose-500/30 animate-fadeIn">
                <span className="text-[10px] font-bold text-rose-500 px-1.5 hidden sm:inline">🛡️ Maintenance</span>
                <button
                  type="button"
                  onClick={() => setIsConfirmingBulkDelete(true)}
                  disabled={isLoading || isDeleting}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold transition-all active:scale-95 shadow-sm"
                  title="Delete all projects permanently from Google Cloud Storage"
                >
        ...
        {/* Navigation Filter Tabs: All vs My Projects */}
        <div className="px-6 sm:px-8 pt-3 pb-0 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 bg-slate-50/50 dark:bg-slate-900/30">
          <button
            type="button"
          ...
          >
            <span>All Team Projects</span>
            <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-[10px] font-extrabold text-slate-700 dark:text-slate-300">
              {jobs.length}
            </span>
          </button>
          <button
          ...
          >
            ...
            {currentLdap && (
              <span className="text-[11px] font-mono text-slate-400">({currentLdap})</span>
            )}
            <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950/80 text-[10px] font-extrabold text-blue-700 dark:text-blue-300">
              {myJobsCount}
            </span>
          </button>
        </div>
        ...
        <div className="p-4 sm:px-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 bg-white dark:bg-slate-900">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={tabFilter === 'my' ? `Search your projects (${currentLdap || 'you'})...` : "Search by customer, owner, or keyword..."}
              value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-4 max-h-[60vh]">
          {isLoading && jobs.length === 0 ? (
            ...
            <div className="text-center py-16 space-y-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-8">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200">
                  {searchQuery ? 'No matching projects found' : tabFilter === 'my' ? 'No projects created by you yet' : 'No projects found'}
                </h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  {searchQuery
                    ? `No projects matching "${searchQuery}". Try a different customer or LDAP search term.`
                    : tabFilter === 'my'
                    ? `You haven't created any countdown projects under LDAP '${currentLdap || 'you'}' yet.`
            ...
            filteredJobs.map((job) => {
              ...
              const isOwnedByMe = isOwnerMatch(job.creatorLdap);
                        ...
                        {job.creatorLdap && (
                          <span
                            className={`text-[11px] font-bold px-2 py-0.5 rounded-md flex items-center gap-1 ${
                              isOwnedByMe
                                ? 'bg-blue-500/15 text-blue-600 dark:text-blue-300 border border-blue-400/40 ring-1 ring-blue-400/30'
                                : 'bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-300 border border-purple-200 dark:border-purple-800'
                            }`}
                          >
                            <span>
                              👤 Owner: {job.creatorLdap}
                              {isOwnedByMe ? ' (You)' : ''}
                            </span>
                          </span>
                        )}
                        {isActive && (
                          <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1 shadow-sm">
                      ...
                      {isConfirmingDelete ? (
                        ...
                        <>
                          ...
                          >
                            ...
                            <span>{isActive ? 'Current in Tab' : 'Open Project'}</span>
                          </button>
        ...
        {/* Bulk Delete All Projects Confirmation Dialog */}
        {isConfirmingBulkDelete && (
          <div className="p-5 border-t border-rose-200 dark:border-rose-900 bg-rose-50/90 dark:bg-rose-950/80 flex flex-col sm:flex-row items-center justify-between gap-4 animate-fadeIn">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-xl bg-rose-200 dark:bg-rose-900 text-rose-700 dark:text-rose-300 shrink-0">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-sm font-bold text-rose-900 dark:text-rose-100">
                  Bulk Delete All {jobs.length} Projects?
                </h4>
                <p className="text-xs text-rose-700 dark:text-rose-300">
                  Permanently deletes all prompt plans, generated images, and Veo 3 videos from Cloud Storage.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeleting}
                onClick={async () => {
                  if (onBulkDeleteAllJobs) {
                    setIsDeleting(true);
                    try {
                      await onBulkDeleteAllJobs();
                ...
                className="px-5 py-2 rounded-xl text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white shadow-md shadow-rose-500/20 transition-all flex items-center gap-1.5"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{isDeleting ? 'Deleting All...' : 'Yes, Delete All'}</span>
              </button>
            </div>
          </div>
        )}
